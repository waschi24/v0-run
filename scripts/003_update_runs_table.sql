...
alter table public.runs add column if not exists duration integer; -- In seconds, e.g., 1800 for 30 minutes
alter table public.runs add column if not exists distance float; -- z.B. 5.0 for 5 km